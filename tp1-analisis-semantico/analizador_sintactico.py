...
from tablaSimbolos import Lexema, Pila, Scope
...
print("Se creo la pila de simbolos: \n" + str(pila))
...
def programa():
    ...
    match_type("id")
    simbolo = Lexema(nombre, "programa", None, linea, None, None)
...
def procedimiento():
    match_val("procedure")
    nombre, linea = preanalisis[1], preanalisis[2]
    scope = Scope(pila.top().nivel + 1, nombre, pila.top())
    pila.push(scope)
    match_type("id")
    procedimiento_prima()
